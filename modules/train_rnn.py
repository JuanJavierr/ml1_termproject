...
if __name__ == "__main__":
    ...
    dataset = build_dataset(
        path="ml1_termproject/lapresse_crawler",
    ...
    X = [
        ...
        if x["section_1"]
        in ["actualites", "sports", "affaires", "arts", "international"]
    ]
    Y = [
        x["section_label"]
        for x in dataset.values()
    ...
    X_train = torch.stack(
        [
            torch.tensor(text_to_word2vec(x, word2vec_model), dtype=torch.float32).view(
                1, -1
            )
            for x in X_train
        ],
